...
class _GamePageState
  ...
  void _initializeGame() {
    ...
    // 5. Schedule settling *after* the first frame
    WidgetsBinding.instance.addPostFrameCallback(
      (
        _,
      ) {
        if (mounted) {
          // Run the settle logic
          _controller.settleBoard(); // This will update the game state

          // NOW find the first lever
          _findNextAvailableLever(
            1,
          ); // This will call setState

          // OK, now we can allow normal updates
  ...
  void _onGameStateChanged() {
    // Don't run setState if we're still initializing
